---
description: >-
  Example code for the eth_getUncleCountByBlockHash JSON-RPC method. Complete
  guide to using the eth_getUncleCountByBlockHash JSON-RPC method in GetBlock.io
  Web3 documentation.
---

# eth\_getUncleCountByBlockHash -Robinhood

Returns the number of uncle blocks for a given block hash.

{% hint style="info" %}
Always `0x0` on Robinhood Chain — Arbitrum Nitro doesn't produce uncle blocks (unlike Ethereum's PoW history). Included for tooling compatibility.
{% endhint %}
...
| ------ | -------- | ------------------------------- |
| result | QUANTITY | Always `0x0` on Robinhood Chain |

## Use Cases

* Compatibility with tools that probe uncle data
...
const result = await provider.send('eth_getUncleCountByBlockHash', ["0x8b5c6f1a9e8d7c6b5a4f3e2d1c0b9a8e7f6d5c4b3a2b1f9e8d7c6b5a4f3e2d1c"]);
console.log(result);
