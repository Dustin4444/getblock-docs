...
description: >-
  ...
  the net_listening JSON-RPC method in GetBlock.io Web3 documentation.
---

# net\_listening - Robinhood

Returns `true` if the client is actively listening for network connections. Usually always `true` on production RPC infrastructure.
...
| ------ | ------- | --------------------------------------------------- |
| result | boolean | `true` if the node is listening for P2P connections |

## Use Cases

* Health check probes for node availability
* Detecting nodes that have lost peer connectivity
