...
description: >-
  ...
  to using the eth_maxPriorityFeePerGas JSON-RPC method in GetBlock.io Web3
  documentation.
---

# eth\_maxPriorityFeePerGas - Robinhood

Returns the suggested priority fee (tip) per gas unit for EIP-1559 transactions, in wei. Note: Robinhood Chain uses first-come-first-served sequencing, so priority fees don't affect transaction ordering — but they do affect inclusion vs delay when the sequencer is congested.
...
| ------ | -------- | ------------------------------------------------------------------------------------------------------ |
| result | QUANTITY | Suggested priority fee per gas in wei (hex). Often `0x0` on Arbitrum Orbit chains due to FCFS ordering |

## Use Cases

* EIP-1559 transaction priority fee selection
* Wallet fee suggestion UIs
