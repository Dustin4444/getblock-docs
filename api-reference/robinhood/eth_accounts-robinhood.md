---
description: >-
  Example code for the eth_accounts JSON-RPC method. Complete guide to using the
  eth_accounts JSON-RPC method in GetBlock.io Web3 documentation.
---

# eth\_accounts - Robinhood

Returns accounts owned by the connected client. On shared RPC infrastructure, this is typically an empty array — wallet/account management is performed client-side via signing libraries.
...
| ------ | ------------- | --------------------------------------------------------------------- |
| result | array of DATA | Accounts known to the node (typically empty on shared infrastructure) |

## Use Cases

* Confirming the node doesn't hold signing keys (security property)
* Compatibility with tools that probe for accounts
